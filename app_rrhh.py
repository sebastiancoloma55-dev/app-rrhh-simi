...
def init_db():
    ...
    # Migración de movimientos para compatibilidad con bases antiguas.
    ensure_columns(cur, "movimientos", {
        "fecha_registro": "TIMESTAMP",
        "usuario_realiza": "TEXT",
        "rut_colaborador": "TEXT",
        "sucursal_origen": "TEXT",
...
# ============================================================
    ...
    if df.empty:
        ...
        if opciones:
            ...
            t1, t2, t3, t4 = st.tabs(["🕐 Asistencia", "🏥 Licencias médicas", "🏖️ Vacaciones y permisos", "🔄 Coberturas"])
            with t1:
            ...
            with t2:
                # Las LM se consultan desde el ausentismo oficial cargado.
                lm_persona = query_df("""
                    SELECT
                        id AS ID,
                        fecha_inicio AS "Fecha inicio",
                        fecha_fin AS "Fecha término",
                        dias AS Días,
                        tipo_ausencia AS "Tipo de ausencia",
                        activo AS Activa,
                        CASE
                            WHEN date(fecha_inicio) <= date(?) AND date(fecha_fin) >= date(?)
                            THEN 'ACTIVA'
                            ELSE 'FINALIZADA'
                        END AS Estado
                    FROM ausentismo_historico
                    WHERE rut=?
                      AND lower(tipo_ausencia) LIKE '%licencia%'
                    ORDER BY date(fecha_inicio) DESC, id DESC
                """, (date.today().isoformat(), date.today().isoformat(), rut))

                if lm_persona.empty:
                    st.info("No hay licencias médicas registradas para este colaborador en el histórico oficial.")
                else:
                    activas = int((lm_persona["Estado"] == "ACTIVA").sum())
                    st.caption(f"🏥 Licencias médicas: {len(lm_persona)} · Activas hoy: {activas}")
                    st.dataframe(lm_persona, use_container_width=True, hide_index=True)

            with t3:
                # Vacaciones y permisos provienen del historial general oficial.
                aus_persona = query_df("""
                    SELECT
                        source_id AS ID,
                        ...
                        tipo AS Tipo,
                        detalle AS Detalle,
                        en_proceso AS "En proceso"
                    FROM solicitudes_historial
                    WHERE rut=?
                    ORDER BY datetime(fecha_solicitud) DESC, source_id DESC
                """, (rut,))

                if aus_persona.empty:
                    st.info("No hay vacaciones o permisos registrados para este colaborador.")
                else:
                    pendientes = int((aus_persona["en_proceso"].astype(str) == "1").sum())
                    st.caption(f"🏖️ Vacaciones/permisos: {len(aus_persona)} · En proceso: {pendientes}")
                    st.dataframe(aus_persona, use_container_width=True, hide_index=True)
            with t4:
                st.dataframe(query_df("""
...
# ============================================================
    ...
    page_title(
        "Coberturas",
        "Registre una cobertura en 5 pasos simples. La sucursal habitual viene desde la columna BM y se puede cambiar.",
    )
    ...
    if colabs.empty or sucs.empty:
        st.warning("⚠️ Cargue primero el Directorio y la lista de colaboradores.")
    else:
        pre = st.session_state.get("preprogramar_cobertura", {})

        # ---------- Paso 1: persona ausente ----------
        st.markdown('<div class="section">1️⃣ ¿Quién está ausente?</div>', unsafe_allow_html=True)

        target_options = colabs["rut"].tolist()
        target_default = target_options.index(pre["rut_reemplaza"]) if pre.get("rut_reemplaza") in target_options else 0
        ...
        st.caption("La sucursal se propone automáticamente según la ubicación registrada del colaborador ausente; puedes cambiarla.")
        ...
        # ---------- Paso 3: quién cubre ----------
        st.markdown('<div class="section">3️⃣ ¿Quién realizará la cobertura?</div>', unsafe_allow_html=True)
        support_options=colabs["rut"].tolist()
        support_default=support_options.index(pre["rut_apoyo"]) if pre.get("rut_apoyo") in support_options else 0

        rut_apoyo=st.selectbox(
            "Colaborador de apoyo",
            support_options,
            index=support_default,
            format_func=lambda r: collaborator_label(r,colabs),
            key="cov_support",
        )
        ...
        if pre:
            try: default_inicio=datetime.strptime(str(pre.get("inicio")),"%Y-%m-%d").date()
            except Exception: pass
            try: default_termino=datetime.strptime(str(pre.get("termino")),"%Y-%m-%d").date()
            except Exception: pass
            if pre.get("motivo"): default_motivo=pre["motivo"]

        st.markdown('<div class="section">4️⃣ ¿Cuándo será la cobertura?</div>', unsafe_allow_html=True)
        c1,c2=st.columns(2)
        inicio=c1.date_input("Desde",value=default_inicio,key=f"cov_ini_{pre.get('ausencia_id','manual')}")
        termino=c2.date_input("Hasta",value=default_termino,key=f"cov_fin_{pre.get('ausencia_id','manual')}")

        motivos=["Cobertura Vacaciones QF","Licencia Médica","Permiso Administrativo","Refuerzo Apertura","Refuerzo Cierre","Otro"]
        m_idx=motivos.index(default_motivo) if default_motivo in motivos else 0
        motivo=st.selectbox("Motivo",motivos,index=m_idx)
        detalle_motivo=st.text_input("Detalle",value="") if motivo=="Otro" else motivo

        # ---------- Paso 5: horario ----------
        st.markdown('<div class="section">5️⃣ ¿Qué horario realizará?</div>', unsafe_allow_html=True)
        st.caption("Marque solo los días que realmente trabajará. El sistema descuenta la colación indicada.")
        dias_sem=["Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"]
        horas=[]
        day_cols=st.columns(4)
        for i,dia in enumerate(dias_sem):
            with day_cols[i % 4]:
                activo=st.checkbox(dia,key=f"cov_day_{i}")
                ent=st.time_input("Entrada",value=time(9,0),key=f"cov_ent_{i}")
                sal=time(18,0)
                sal=st.time_input("Salida",value=sal,key=f"cov_sal_{i}")
                if activo:
                    horas.append((dia,ent,sal))
        colacion=st.number_input("Colación (minutos)",min_value=0,max_value=180,value=30,step=5)

        # ---------- Vista previa ----------
        if horas:
            total=sum(net_hours(e,sal,colacion) for _,e,sal in horas)
            st.markdown('<div class="section">👁️ Vista previa antes de guardar</div>',unsafe_allow_html=True)
            a,b,c=st.columns(3)
            a.metric("Persona cubierta",str(target_row["nombre_completo"])[:28])
            b.metric("Persona de apoyo",str(support_row["nombre_completo"])[:28])
            c.metric("Horas configuradas",f"{total:.2f} h")
            st.info(
                f"📍 {sucs.loc[sucs['codigo']==suc_dest,'nombre'].iloc[0]} · "
                f"📅 {inicio:%d/%m/%Y} → {termino:%d/%m/%Y} · "
                f"⏱️ {', '.join(d[:3] for d,_,_ in horas)}"
            )
        else:
            total=0
        ...
        if st.button("💾 GUARDAR COBERTURA",type="primary",use_container_width=True):
            if termino < inicio:
                ...
                st.error("❌ Seleccione al menos un día de cobertura.")
            elif str(rut_apoyo)==str(rut_reemplaza):
                st.error("❌ El colaborador de apoyo no puede ser la misma persona ausente.")
            else:
                total=sum(net_hours(e,sal,colacion) for _,e,sal in horas)
                detalle_horario=" | ".join(f"{d[:3]} {e:%H:%M}-{sal:%H:%M}" for d,e,sal in horas)
                detalle_horario += f" | {total:.2f} h netas | Colación {colacion} min | BM apoyo {support_bm or '-'} | BM ausente {target_bm or '-'}"
                ...
                """,(
                    ...
                    suc_origen,suc_dest,rut_reemplaza,inicio.isoformat(),
                    termino.isoformat(),detalle_horario,detalle_motivo
                ))
                audit("COBERTURA",f"{rut_apoyo} {suc_origen}->{suc_dest} cubre {rut_reemplaza} {inicio}->{termino}")
                st.cache_data.clear()
                st.session_state.preprogramar_cobertura={}
                st.success("✅ Cobertura guardada correctamente.")
                st.rerun()
        ...
        df=query_df("""
            SELECT
                ...
                c.nombre_completo AS "Colaborador de apoyo",
                COALESCE(so.nombre,m.sucursal_origen,'Sin origen') AS "Sucursal origen",
                COALESCE(sd.nombre,m.sucursal_destino,'Sin destino') AS "Sucursal a cubrir",
                COALESCE(c2.nombre_completo,m.rut_reemplazado,'Sin persona') AS "Persona cubierta",
                m.fecha_inicio AS Desde,
                m.fecha_fin AS Hasta,
                m.nuevo_horario AS Horario,
                m.motivo AS Motivo
            FROM movimientos m
